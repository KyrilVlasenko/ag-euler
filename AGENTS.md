...
- Frax: `contracts/frax-contracts/frax-AGENTS.md`
...
Key decisions documented in `contracts/balancer-contracts/balancer-AGENTS.md`:
...
3. **MonadScan verification uses Etherscan V2 API, not Sourcify.** Sourcify only shows on MonadVision. For MonadScan: `curl -X POST "https://api.etherscan.io/v2/api?chainid=143"` with Standard JSON Input from `forge verify-contract --show-standard-json-input`. `forge verify-contract --chain 143 --verifier etherscan` fails — forge doesn't know chain 143. See `contracts/balancer-contracts/balancer-AGENTS.md` lesson #23.
